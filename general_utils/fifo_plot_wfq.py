...
if(len(sys.argv[1:]) < num_args):
    print "usage <base_dir> <prefix> <output_dir>"

base_dir = sys.argv[1] 
pre = sys.argv[2] 
dir = sys.argv[3]
fname = base_dir + '/' + pre + ".out"
f = open(fname)
...
plt.savefig('%s/%s.%s.png' %(dir,pre,"known_fifo_1_queue_occupancy"))
...
plt.legend(loc='lower right')
plt.savefig('%s/%s.%s.png' %(dir,pre,"known_fifo_2_queue_occupancy"))
plt.draw()
...
plt.savefig('%s/%s.%s.png' %(dir,pre,"unknown_load"))
plt.draw()
...
plt.legend(loc='upper right')
plt.savefig('%s/%s.%s.png' %(dir,pre,"rates"))
plt.draw()
...
plt.ylabel('Rates in Mbps')
plt.legend(loc='upper right')
plt.savefig('%s/%s.%s.png' %(dir,pre,"destination_rates"))
plt.draw()
...
plt.ylabel('Rates in Mbps')
plt.legend(loc='upper right')
plt.savefig('%s/%s.%s.png' %(dir,pre,"destination_rates_perpacket"))
plt.draw()
