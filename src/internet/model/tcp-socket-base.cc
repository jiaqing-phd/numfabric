                            ...
                            Ptr<Ipv4Interface> incomingInterface)
{
  ...
  Address toAddress = InetSocketAddress (header.GetDestination (), m_endPoint->GetLocalPort ());
   ...
   // Get the ECN bit from the IP header
  Ipv4Header::EcnType ecn_info = header.GetEcn();
  // NS_LOG_UNCOND("TcpSocketBase::DoForwardUp .. ECN INFO extracted from IP header "<<header.EcnTypeToString(ecn_info));


  /* receiver functionality */
  if(ecn_info == Ipv4Header::ECN_CE) {
    /* On an ACK, we need to set the ECN bit in the header */
    need_echo = true;
 //   NS_LOG_UNCOND("need_echo is True now");
  }
...
//  if(p->GetSize() > 500) { /* TBD : what is the size of ack, syn, fin? */
  ...
  if (!m_rxBuffer.Add (p, tcpHeader))
    { // Insert failed: No data or RX buffer full
      SendEmptyPacket (TcpHeader::ACK | ce_flag);
      return;
    }
...
void
//TcpSocketBase::DelAckTimeout (TcpHeader::Flags_t ceflags)
TcpSocketBase::DelAckTimeout (void)
{
  m_delAckCount = 0;
  //if (ceflags & TcpHeader::ECE) {
