...
epoch_time = 0.05 #50 ms
sample_time = 0.0001
window = 10

def find_change(epoch_num):
  flow_changed = {}
  old_rate = {}
  new_rate = {}
  ...
  for line in f:
    ...
    enum = int(parts[0])
    flowid = int(parts[1])
    ideal_rate = float(parts[2])

    if(enum == 1):
      # first epoch - every flow rate is considered changed
      flow_changed[flowid] = 1
      continue
    ## otherwise
    if(enum == epoch_num -1):
      old_rate[flowid] = ideal_rate
    if(enum == epoch_num):
      new_rate[flowid] = ideal_rate
...
def below_10(sts, thresh):
    ...
    while(end < len(sts)):
        ...
        while(abs(sts[start+index])<thresh and index<window):
...
def get_epoch_data(epoch_num, flows_changed):
    total_sent = {}
    ideal_sent = {}
    alg_sent = {}
    flow_diff_times = {}
