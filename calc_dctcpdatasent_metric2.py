...
def find_change(epoch_num, old_rate, new_rate):
  ...
  for flowid in new_rate:
    if(epoch_num == 1):
      flow_changed[flowid] = 1
      continue
    else:
      # if this is a new flow then you won't have a old_rate for this
      if(flowid in old_rate):
        change = abs(old_rate[flowid]-new_rate[flowid])/old_rate[flowid]
        if(change > 0.1):
          flow_changed[flowid] = 1
#      else:
#        print("epoch %d flow %d newrate %f oldrate %f" %(epoch_num, flowid, new_rate[flowid], old_rate[flowid]))

  return flow_changed

# window of few time-slots
def below_10(sts, thresh):
    start = 0
    end = window
    ...
    while(end < len(sts)):
        ...
        index = 0
        while((sts[start+index])<thresh and index<window):
            index +=1
    ...
    print("returning %f" %(converged_time))
    return (converged_time)
...
def get_all_idealrates():
    ideal_rates = {}
    for epochs in range(1, max_epochs):
      ideal_rates[epochs] = get_ideal_rates(epochs)
    return ideal_rates

def get_epoch_data(epoch_num, ideal_rates, flows_changed):
    total_sent = {}
    ideal_sent = {}
    alg_sent = {}
    flow_diff_times = {}
    ...
    for line in f:
     ...
     if(xy[0] == "sinkdata"):
      ...
      if(ideal_rate == 0):
          continue
      if(fid not in flows_changed):
          print("epoch %f fid %d no change "%(epoch_num, fid))
          continue
      ...
      if(ideal_sent[fid] > 0.0):
        diff = abs(ideal_sent[fid] - alg_sent[fid])/ideal_sent[fid]
...
ideal_rates = get_all_idealrates()
ninety_fifths = []
for epochs in range(1, max_epochs):
   if(epochs == 1):
     flows_changed = find_change(epochs, None, ideal_rates[1])
   else:
     flows_changed = find_change(epochs, ideal_rates[epochs-1], ideal_rates[epochs])

   print("epoch %d flows_changed %d" %(epochs, len(flows_changed)))

   (flow_diff_times, flow_diffs, flow_data) = get_epoch_data(epochs, ideal_rates[epochs], flows_changed)
   print("Returning with epoch %d data %d %d %d" %(epochs, len(flow_diff_times), len(flow_diffs), len(flow_data)))

   # when did 95% of them come down to 10% and stay there for 10 instances

   for thresh in (0.1, 0.01, 0.05):
     below_10_data = []
     below_10_time = []
     for key in flow_diffs:
       if(key not in flows_changed):
         continue
       fdata = flow_data[key]
       t = below_10(flow_diffs[key], thresh)
       ...
       print("rawconvergedata thresh %f flow %d converge_data %f converged_time %f epoch %d" %(thresh, key,data_sent,time_sent,epochs))
     ...
     print("thresh %f epoch %d ninety_fifth_data %f ninety_fifth_time %f median_data %f median_time %f" %(thresh, epochs, ninety_fifth_data, ninety_fifth_time, median_data, median_time))
