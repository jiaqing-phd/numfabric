...
/* 
Ptr<MyApp> startFlow(uint32_t sourceN, uint32_t sinkN, double flow_start, uint32_t flow_size, uint32_t flow_id, NodeContainer clientNodes, uint32_t rand_weight)
{
  port++;
  // Socket at the source
  Ptr<Ipv4L3Protocol> sink_node_ipv4 = StaticCast<Ipv4L3Protocol> ((clientNodes.Get(sinkN))->GetObject<Ipv4> ());
...
//  for (uint32_t i=1; i<=max_flows; i++)
    while(true) 
    {
     ...
     if(flow_started[i] == 0) { 
       ...
       flow_started[i] = 1;
       std::cout<<Simulator::Now().GetSeconds()<<" starting flow "<<i<<" source "<<source_node<<" sink_node "<<sink_node<<" ( "<<(clientNodes.Get(source_node))->GetId()<<" , "<<(clientNodes.Get(sink_node))->GetId()<<" )"<<std::endl;
       num_flows++;
       break;
     }
   }
...
//    for(uint32_t i=0; i<2; i++) {
        ...
        UniformVariable randt;
//        uint32_t rand_weight = randt.GetInteger(1, 12);
//        uint32_t rand_weight = weights[run_num][i];
        uint32_t rand_weight = 1.0;
//      double static_weights[] = {1.0, 1.0, 2.0, 6.0, 3.0, 1.0, 1.0};
     
        if(flow_started[i] == 0) { 
          Ptr<MyApp> sendingApp = startFlow(source_node, sink_node, flow_start, flow_size, global_flowid, clientNodes, rand_weight);
          sending_apps[i] = sendingApp;
          global_flowid++;
          flow_started[i] = 1;
...
//  Config::SetDefault ("ns3::RttEstimator::MinRTO", TimeValue(Seconds(0.001)));
  ...
  bottleneckNodes.Create (11);
  clientNodes.Create (33);
  ...
  p2paccess.SetDeviceAttribute ("DataRate", StringValue ("70Gbps"));
  ...
  bdevice.push_back(p2pbottleneck.Install(bottleneckNodes.Get(0), bottleneckNodes.Get(2)));
  printlink(bottleneckNodes.Get(0), bottleneckNodes.Get(2));
  bdevice.push_back(p2pbottleneck.Install(bottleneckNodes.Get(0), bottleneckNodes.Get(3)));
  printlink(bottleneckNodes.Get(0), bottleneckNodes.Get(3));
  ...
  bdevice.push_back(p2pbottleneck.Install(bottleneckNodes.Get(2), bottleneckNodes.Get(5)));
  printlink(bottleneckNodes.Get(2), bottleneckNodes.Get(5));
  ...
  bdevice.push_back(p2pbottleneck.Install(bottleneckNodes.Get(6), bottleneckNodes.Get(9)));
  printlink(bottleneckNodes.Get(6), bottleneckNodes.Get(9));
  ...
  bdevice.push_back(p2pbottleneck.Install(bottleneckNodes.Get(8), bottleneckNodes.Get(10)));
  printlink(bottleneckNodes.Get(8), bottleneckNodes.Get(10));

  // Attach other nodes: 2 access nodes per link
  ...
  access.push_back(p2paccess.Install(bottleneckNodes.Get(1), clientNodes.Get(2)));
  ...
  access.push_back(p2paccess.Install(bottleneckNodes.Get(3), clientNodes.Get(6)));
  access.push_back(p2paccess.Install(bottleneckNodes.Get(3), clientNodes.Get(7)));


  access.push_back(p2paccess.Install(bottleneckNodes.Get(4), clientNodes.Get(8)));
  access.push_back(p2paccess.Install(bottleneckNodes.Get(4), clientNodes.Get(9)));
  ...
  access.push_back(p2paccess.Install(bottleneckNodes.Get(6), clientNodes.Get(12)));
  access.push_back(p2paccess.Install(bottleneckNodes.Get(6), clientNodes.Get(13)));

  access.push_back(p2paccess.Install(bottleneckNodes.Get(7), clientNodes.Get(14)));
  access.push_back(p2paccess.Install(bottleneckNodes.Get(7), clientNodes.Get(15)));

  access.push_back(p2paccess.Install(bottleneckNodes.Get(8), clientNodes.Get(16)));
  access.push_back(p2paccess.Install(bottleneckNodes.Get(8), clientNodes.Get(17)));

  access.push_back(p2paccess.Install(bottleneckNodes.Get(9), clientNodes.Get(18)));
  access.push_back(p2paccess.Install(bottleneckNodes.Get(9), clientNodes.Get(19)));

  access.push_back(p2paccess.Install(bottleneckNodes.Get(10), clientNodes.Get(20)));
  access.push_back(p2paccess.Install(bottleneckNodes.Get(10), clientNodes.Get(21)));
  ...
  //static const uint32_t arr1[] = {0, 0,4,1,10,19,2,6,9,2,10, 11,13,2,1,0,4,13,13};
  static const uint32_t arr1[] = {0, 0,4,1,10,19,2,6,9,2,10,11,  13,2,1,0,4,4,13,13,1};
  std::vector<uint32_t> sourcenodes (arr1, arr1 + sizeof(arr1) / sizeof(arr1[0]) );
  
  //static const uint32_t arr[] = {0, 10,12,14,18,16,8,17,17,19,20,14,16,15,19,16,8,15,17,14}; 
  static const uint32_t arr[] = {0, 10,12,14,18,16,8,17,17,19,20,14,  16,15,19,16,8,15,17,14,9}; 
