...
//  std::vector<uint32_t> sourcenodes {3};
  uint32_t flow_id = 1; 
/*
  while(flow_id < 6) 
  {
    for(uint32_t i=0; i < sourcenodes.size(); i++) 
    {
      ...
      //startflowwrapper(i, sourcenodes, sinknodes, clientNodes, flow_id);
      uint32_t source_node = sourcenodes[i];
      uint32_t sink_node = sinknodes[i];

      double flow_size = SOME_LARGE_VALUE;
      double flow_start = 1.0;
      
      
      UniformVariable randt(1,8);
      double rand_weight = randt.GetValue();
//      double static_weights[] = {1.0, 1.0, 2.0, 6.0, 3.0, 1.0, 1.0};
      
      startFlow(source_node, sink_node, flow_start, flow_size, flow_id, clientNodes, rand_weight);
      flow_started[i] = 1;
   ...
   */
   ...
   Simulator::Schedule (Seconds (1.0), &startflowwrapper, sourcenodes, sinknodes, clientNodes, flow_id);
